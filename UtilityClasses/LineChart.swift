...
open class LineChart: UIView {
    ...
    */
    ...
    public struct Axis {
        public var visible: Bool = true
        // #607d8b
        public var color: UIColor = UIColor(red: 169/255.0, green: 169/255.0, blue: 169/255.0, alpha: 0.8)
     ...
     */
    open func highlightDataPoints(_ index: Int) {
    ...
    fileprivate func drawDataDots(_ lineIndex: Int) {
        ...
        data.removeLast()  // remove last value (max value)
        ...
        for index in 0..<data.count {
            ...
            // add max and min line
            let ymaxValue = self.bounds.height - self.y.scale(maxValue) - y.axis.inset
            let yminValue = self.bounds.height - self.y.scale(minValue) - y.axis.inset
            if index == 0 {
                let lblHigherRange = UILabel(frame: CGRect(x: (UIScreen.main.bounds.width - 150)  , y: ymaxValue - 15 , width: 150, height: 15))
                lblHigherRange.text = String(format:"HIGHER RANGE (%.2f)",maxValue)
                lblHigherRange.textColor = UIColor.green
                lblHigherRange.font = UIFont().smallFont
                self.addSubview(lblHigherRange)
                

                let lblMaxLine = UILabel(frame: CGRect(x: 25, y: ymaxValue, width: line_Width + 300, height: 1))
                lblMaxLine.backgroundColor = UIColor.green
                self.addSubview(lblMaxLine)
                
                let lblLowerRange = UILabel(frame: CGRect(x: 30   , y: yminValue - 15 , width: 150, height: 15)) // (UIScreen.mainScreen().bounds.width-160)
                lblLowerRange.font = UIFont().smallFont
                lblLowerRange.textColor = UIColor.yellow
                lblLowerRange.text = String(format:"LOWER RANGE (%.2f)",minValue)
                self.addSubview(lblLowerRange)
                
                
                let lblMinLine = UILabel(frame: CGRect(x: 25, y: yminValue , width: line_Width + 300, height: 1))
                lblMinLine.backgroundColor = UIColor.yellow
                self.addSubview(lblMinLine)
                
//                //set higher & lower value on buttom 
//                let lblBottomHigher = BaseUIController().ALabelFrame(CGRectMake((UIScreen.mainScreen().bounds.width - 250)/2 ,self.bounds.height - 20 , 120, 15), withString:  String(format:"HIGHER RANGE (%.2f)",maxValue))as! UILabel
//                lblBottomHigher.textColor = UIColor.greenColor()
//                lblBottomHigher.textAlignment = .Center
//                lblBottomHigher.font = UIFont().verySmallFont
//                self.addSubview(lblBottomHigher)
//                
//                let lblBottomLower = BaseUIController().ALabelFrame(CGRectMake(lblBottomHigher.frame.origin.x + 130 ,self.bounds.height - 20 , 120, 15), withString: String(format:"LOWER RANGE (%.2f)",minValue))as! UILabel
//                lblBottomLower.textColor = UIColor.greenColor()
//                lblBottomLower.textAlignment = .Center
//                lblBottomLower.font = UIFont().verySmallFont
//                self.addSubview(lblBottomLower)
            ...
            dotLayer.dotInnerColor = UIColor(red: 195/255.0, green: 195/255.0, blue: 195/255.0, alpha: 1) //colors[lineIndex]
    ...
    fileprivate func drawAxes() {
        let height = self.bounds.height + 10 // add by me
        ...
        path.addLine(to: CGPoint(x: x.axis.inset, y: 15)) //y.axis.inset add by me (15)
        path.stroke()
    }
    ...
    fileprivate func drawLine(_ lineIndex: Int) {
        ...
        var data = self.dataStore[lineIndex]
        data.removeLast() //  remove last added max value 
        let path = UIBezierPath()
...
//        }
    ...
    fileprivate func drawXLabels() {
        ...
        let y = self.bounds.height - x.axis.inset + 10
        ...
        for (index, _) in xAxisData.enumerated() {
          //  var xValue :CGFloat =  10 //self.x.scale(CGFloat(index)) + x.axis.inset - (width / 2)
            let label = UILabel.init(frame: CGRect(x: xValue, y: y, width: 70, height: x.axis.inset - 10)) //UILabel(frame: CGRect(x: xValue, y: y, width: width + 20, height: x.axis.inset - 10))
            label.font = UIFont().smallFont//UIFont.preferredFontForTextStyle(UIFontTextStyleCaption2)
           ...
           // label.backgroundColor = UIColor.yellowColor()
            ...
            label.text = text
            xValue += (UIScreen.main.bounds.width)/3
    ...
    fileprivate func drawYLabels() {
        ...
        for i in stride(from: start, through: stop, by: step){
            ...
            // hindustan wellness
            if stop <= 4 {
                label.text = String(Float(i))
            }else{
                label.text = String(Int(i))
            }
...
//    public func drawMax_MinLine(max : Int , min : Int) {
//        var yValue: CGFloat
//        let (start, stop, step) = self.y.ticks
//        for var i: CGFloat = start; i <= stop; i += step {
//            yValue = self.bounds.height - self.y.scale(i) - (y.axis.inset * 1.5)
//            let label = UILabel(frame: CGRect(x: 0, y: yValue, width: 320, height: y.axis.inset))
//            label.backgroundColor = UIColor.yellowColor()
//            self.addSubview(label)
//        }
//    }
     ...
     * Add line chart
     */
    open func addLine(_ data: [CGFloat] , min_Value : CGFloat , max_Value : CGFloat , line_width : CGFloat , testName : String) {
        minValue = min_Value
        maxValue = max_Value
        test_Name = testName
...
//        }
        ...
        let start = extent[0]//ceil(extent[0] / step) * step
        let stop =  extent[1]//floor(extent[1] / step) * step + step * 0.5 // inclusive
