//
//  MyRecordsDietChartsViewController.swift
//  Health
//
...
class MyRecordsDietChartsViewController: UIViewController,UITableViewDelegate , UITableViewDataSource ,serverTaskComplete , CustomDelegate {
    ...
    var txtDietitianName = UITextField()
...
//        mainBGView.addGestureRecognizer(tapped)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        var lblDietitianName = UILabel()
        var lblDate = UILabel()

        lblDietitianName = BaseUIController().ALabelFrame(CGRect(x: image_width + 10, y: yPos, width: self.view.frame.width-image_width, height: lblHeight), withString: "") as! UILabel
        yPos += lblHeight
        lblDate = BaseUIController().ALabelFrame(CGRect(x: image_width + 10, y: yPos, width: 200.0, height: lblHeight), withString: "") as! UILabel
        lblDietitianName.font = UIFont().mediumFont
        lblDate.font = UIFont().mediumFont
        let test = reportObj.dietitianName  //arrFilterRecords![indexPath.row].valueForKey("dietitianname")as? String
        lblDietitianName.text = String(format: "Dietitian: %@",test)
        let date =  reportObj.reportDate //arrFilterRecords![indexPath.row].valueForKey("chartdate")as? String
        lblDate.text = String(format: " Date: %@",date)
        
        cell.addSubview(lblDietitianName)
        ...
        let tapped:UITapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(MyRecordsDietChartsViewController.tappedOnImage(_:)))
        tapped.numberOfTapsRequired = 1
    ...
    func hitAWebServices()  {
        ...
        if Reachability.isConnectedToNetwork() == true {
            ...
            let allParameters = ["customerId":customerId, "recordType":"D"]
...
//        DispatchQueue.global(priority: DispatchQueue.GlobalQueuePriority.default).async(execute: {
    ...
    func syncServertoLocalDB(_ arrMyRecord : NSArray)  {
        ...
        for var i in (0..<arrMyRecord.count) {
            ...
            do {
                ...
                let dietitianname = (arrMyRecord[i] as AnyObject).value(forKey: "dietitianname")as! String
                let date = (arrMyRecord[i] as AnyObject).value(forKey: "chartdate")as! String
                ...
                let rs = try database.executeQuery(String(format:"select * from recordDetails where dataBaseId = %@  and RecordType = 'D'",dataBaseId), values: nil)
                if rs.next() == false {
                try database.executeUpdate("insert into RecordDetails (DataBaseId , MemberId ,RecordId, RecordDate ,DietitianName ,RecordLabName , RecordTestName , DoctorName, RecordDisease ,RecordString ,RecordLink,ServerToLocalSyncStatus,LocalToServerSyncStatus, RecordType,isDeleted) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", values: [dataBaseId , memberId ,"0", date, dietitianname ,"","","","","","", "N","Y","D","N"])
                }
                i += 1
    ...
    func loadDataFromDataBase(_ order : String)  {
        // show sorting button after load data
        ...
        do {
            let rs = try database.executeQuery(String(format: "select * from RecordDetails where RecordType = 'D' and isDeleted = 'N' order by _id %@, recorddate %@",order,order), values: nil)
            while rs.next() {
    ...
    func btnSaveOnClick(_ button : UIButton)  {
        ...
        do {
            ...
            let dietitianName = txtDietitianName.text
            let Date = txtDate.text
            try database.executeUpdate(String(format:"update recordDetails set  DietitianName = '%@', RecordDate = '%@',LocalToServerSyncStatus = 'N' where (DataBaseId = '%@' OR RecordId ='%@')  and RecordType = 'D'", dietitianName!,Date!,selectedDatabaseID,selectedDatabaseID), values: nil)
        }
        ...
        if arrCheckUncheck.count > 0 {
            for  i in (0..<arrCheckUncheck.count) {
                ...
                do {
                    ...
                    if recordId == "0" {
                        try database.executeUpdate(String(format:"update recordDetails set  isDeleted = 'Y' where DataBaseId = '%@' and RecordType = 'D'",dataBaseId ), values: nil)
                    }else{
                        try database.executeUpdate(String(format:"update recordDetails set  isDeleted = 'Y' where RecordId = '%@' and RecordType = 'D'", recordId), values: nil)
                    }
                } catch let error as NSError {
                    print("failed: \(error.localizedDescription)")
                }
            ...
            if let urlString = urlWhats.addingPercentEncoding(withAllowedCharacters: CharacterSet.urlQueryAllowed) {
                if let whatsappURL = URL(string: urlString) {
                    ...
                    if UIApplication.shared.canOpenURL(whatsappURL) {
                        let tempFile = URL(fileURLWithPath: NSHomeDirectory()).appendingPathComponent("Documents/DietChart_Recored.pdf")
                        do {
    ...
    func showEditView(_ totalTextField : NSArray , textValue : HealthInfo)  {
        ...
        mainBGView.tag = 400
        mainBGView.backgroundColor = UIColor.black.withAlphaComponent(0.5)
        editPopupView = UIView(frame: CGRect(x: 10, y:(self.view.window!.frame.height/2 - (68)), width: self.view.window!.frame.width - 20, height: 200.0));
        editPopupView.backgroundColor = UIColor.white
        editPopupView.layer.cornerRadius = 2.0
        ...
        for i in (0..<totalTextField.count) {
            ...
            let lblLine = BaseUIController().ALabelFrame(CGRect(x: xPos , y: yPos + 38 , width: editPopupView.frame.width, height: 1), withString: "")as! UILabel
       ...
       // self.view.addSubview(mainBGView)
        ...
        txtDietitianName = editPopupView.viewWithTag(200) as! UITextField
        txtDate = editPopupView.viewWithTag(201) as! UITextField
        txtDietitianName.text = textValue.dietitianName
