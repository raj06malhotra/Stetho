//
//  MyRecordPrescriptionViewController.swift
//  Health
//
...
class MyRecordPrescriptionViewController: UIViewController , UITableViewDelegate , UITableViewDataSource ,serverTaskComplete,CustomDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        recordObj = (arrFilterRecords.object(at: (indexPath as NSIndexPath).section)) as! HealthInfo
        
        let cell:UITableViewCell=UITableViewCell(style: UITableViewCellStyle.subtitle, reuseIdentifier: "cell")
        
        var lblDoctorName = UILabel()
        var lblDiseaseName = UILabel()
        var lblDate = UILabel()
        lblDoctorName = BaseUIController().ALabelFrame(CGRect(x: image_height + 10, y: yPos, width: self.view.frame.width-70, height: lblHeight), withString: "") as! UILabel
        yPos += lblHeight
        
        lblDiseaseName = BaseUIController().ALabelFrame(CGRect(x: image_height + 10, y: yPos, width: 200.0, height: lblHeight), withString: "") as! UILabel
        yPos += lblHeight
        lblDate = BaseUIController().ALabelFrame(CGRect(x: image_height + 10, y: yPos, width: 200.0, height: lblHeight), withString: "") as! UILabel
        lblDoctorName.font = UIFont().mediumFont
        lblDiseaseName.font = UIFont().mediumFont
        lblDate.font = UIFont().mediumFont
        
        let test = recordObj.doctorName
        lblDoctorName.text = String(format: "Doctor: %@",test)
        
        let lab = recordObj.diseaseName
        lblDiseaseName.text = String(format: "Disease: %@",lab)
        
        let time = recordObj.reportDate
        lblDate.text = String(format: " Date: %@",time)
        
        cell.addSubview(lblDoctorName)
        ...
        pdfImageview.tag = (indexPath as NSIndexPath).row
        ...
        let tapped:UITapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(MyRecordPrescriptionViewController.tappedOnImage(_:)))
        tapped.numberOfTapsRequired = 1
    ...
    func hitAWebServices()  {
        ...
        if Reachability.isConnectedToNetwork() == true {
            ...
            let allParameters = ["customerId":customerid, "recordType":"P"]
    ...
    //MARK: customDelegate
    //its called when coming from after editing data report
...
//        tableView.reloadData()
    ...
    func syncServertoLocalDB(_ arrMyRecord : NSArray)  {
        ...
        for var i in (0..<arrMyRecord.count) {
            ...
            do {
                ...
                let date = (arrMyRecord[i] as AnyObject).value(forKey: "prescriptiondate")as! String
                let doctorName = (arrMyRecord[i] as AnyObject).value(forKey: "doctor_name")as! String
                let dataBaseId = (arrMyRecord[i] as AnyObject).value(forKey: "recordid")as! String
                
                print(String(format:"select * from recordDetails where dataBaseId = %@  and RecordType = 'P'",dataBaseId))
                let rs = try database.executeQuery(String(format:"select * from recordDetails where dataBaseId = %@  and RecordType = 'P'",dataBaseId), values: nil)
                
                if rs.next() == false {
                    try database.executeUpdate("insert into RecordDetails (DataBaseId , MemberId ,recordId, RecordDate , DoctorName , RecordDisease ,RecordLabName , RecordTestName,DietitianName,RecordString ,RecordLink ,ServerToLocalSyncStatus,LocalToServerSyncStatus, RecordType,isDeleted) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", values: [dataBaseId , memberId ,"0", date,doctorName, diseaseName ,"","","","","","N","Y","P","N"])
                i += 1
                }
    ...
    func loadDataFromDataBase(_ order : String)  {
        ...
        do {
            let rs = try database.executeQuery(String(format:"select * from RecordDetails where RecordType = 'P' and isDeleted = 'N' order by _id %@, recorddate %@",order,order), values: nil)
            while rs.next() {
    ...
    func btnSaveOnClick(_ button : UIButton)  {
        ...
        do {
            ...
            let doctorName = txtDoctorName.text
            let diseaseName = txtDiseaseName.text
            let Date = txtDate.text
            
          
            
            try database.executeUpdate(String(format:"update recordDetails set  DoctorName = '%@', RecordDisease = '%@', RecordDate = '%@', LocalToServerSyncStatus = 'N' where (DataBaseId = '%@' OR RecordId ='%@')  and RecordType = 'P'", doctorName!,diseaseName!,Date!,selectedDatabaseID,selectedDatabaseID), values: nil)
        ...
        if longPressGestureRecognizer.state == UIGestureRecognizerState.began {
            ...
            if tableView.indexPathForRow(at: touchPoint) != nil {
                ...
                var  selectedReportObj = HealthInfo()
                 selectedReportObj = arrFilterRecords.object(at: indexPath!.section) as! HealthInfo
//                selectedReportObj = (arrFilterRecords.object(at: ((indexPath as NSIndexPath?)?.section)!))! as! HealthInfo
        ...
        if arrCheckUncheck.count > 0 {
            for i in (0..<arrCheckUncheck.count) {
                ...
                do {
                    ...
                    if recordId == "0" {
                        try database.executeUpdate(String(format:"update recordDetails set  isDeleted = 'Y' where DataBaseId = '%@' and RecordType = 'P'",dataBaseId ), values: nil)
                    }else{
                        try database.executeUpdate(String(format:"update recordDetails set  isDeleted = 'Y' where RecordId = '%@' and RecordType = 'P'", recordId), values: nil)
                    }
                } catch let error as NSError {
                    print("failed: \(error.localizedDescription)")
                }
            ...
            if let urlString = urlWhats.addingPercentEncoding(withAllowedCharacters: CharacterSet.urlQueryAllowed) {
                if let whatsappURL = URL(string: urlString) {
                    ...
                    if UIApplication.shared.canOpenURL(whatsappURL) {
                        let tempFile = URL(fileURLWithPath: NSHomeDirectory()).appendingPathComponent("Documents/Prescription_recored.pdf")
                        do {
    ...
    func showEditView(_ totalTextField : NSArray , textValue : HealthInfo)  {
        ...
        mainBGView.tag = 400
        mainBGView.backgroundColor = UIColor.black.withAlphaComponent(0.5)
        editPopupView = UIView(frame: CGRect(x: 10, y: (self.view.window!.frame.height/2 - (100)), width: self.view.window!.frame.width - 20, height: 200.00));
        ...
        btnSave.addTarget(self, action: #selector(MyRecordPrescriptionViewController.btnSaveOnClick(_:)), for: .touchUpInside)
        ...
        txtDoctorName = editPopupView.viewWithTag(200) as! UITextField
        txtDiseaseName = editPopupView.viewWithTag(201) as! UITextField
        txtDate = editPopupView.viewWithTag(202) as! UITextField
        txtDoctorName.text = textValue.doctorName
    ...
    func keyboardWillShow(_ notification:Notification){
        ...
        editPopupView.frame = CGRect(x: 10, y: (UIScreen.main.bounds.height/2 - (180)), width: UIScreen.main.bounds.width-20, height: 200.00)
    }
    
    func keyboardWillHide(_ notification:Notification){
        editPopupView.frame = CGRect(x: 10, y: (UIScreen.main.bounds.height/2 - (100)), width: UIScreen.main.bounds.width - 20 , height: 200.00)
